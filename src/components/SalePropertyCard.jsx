import propertyImage from "../assets/images/propertyImage.jpg";

export function SalePropertyCard() {
  return (
    <div
      id="cardBody"
      className="flex   w-[1024px] border border-cyan-100 rounded-lg shadow-xl"
    >
      <div id="col1" className="w-[330px] p-4  border-cyan-600">
        <div id="image" className="">
          <img
            src={propertyImage}
            alt="Property Image"
            className="w-full object-cover rounded-xl"
          />
        </div>

        <div id="details" className="text-sm pt-4">
          <div className="flex font-semibold">
            <div className="text-start">Owner: Vivek Dethe</div>
            <div className="pl-20">INR.55 lakh</div>
          </div>

          <div className="text-start pt-2">Contact: 9028003579</div>
          <div className="text-start pt-16">4.5km from Katraj</div>
        </div>
      </div>

      <div id="col2" className="pl-6">
        <div className="flex pb-6 pt-8">
          <div id="heading" className="font-semibold text-lg">
            2 BHK Apartments for Sale in Windsor County, Ambegaon Pune
          </div>

          <div id="icons" className="">
            <div>
              <link
                rel="stylesheet"
                href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css"
                integrity="sha512-DTOQO9RWCH3ppGqcWaEA1BIZOC6xxalwEsw9c2QQeAIftl+Vegovlnee1c9QX4TctnWMn13TZye+giMm8e2LwA=="
                crossorigin="anonymous"
                referrerpolicy="no-referrer"
              />
            </div>
            <div className="justify-end items-center pl-8">
              <i class="fa-regular fa-heart mr-3 hover:cursor-pointer"></i>
              <i class="fa-solid fa-share mr-3 hover:cursor-pointer"></i>
              <i class="fa-solid fa-ellipsis-vertical hover:cursor-pointer"></i>
            </div>
          </div>
        </div>

        <div id="icon" className="w-[550px] h-[100px]  border-cyan-600">
          <div id="row1" className="flex py-2 px-1 justify-evenly">
            <div id="icon1" className="flex">
              <i class="fa-solid fa-couch fa-2x mr-3"></i>

              <div>
                <div className="text-sm font-semibold">Furnishing</div>
                <div className="text-xs">Furnished</div>
              </div>
            </div>

            <div id="icon2" className="flex">
              <i class="fa-solid fa-bath fa-2x mr-3"></i>
              <div>
                <div className="text-sm font-semibold">Bathroom</div>
                <div className="text-xs">1</div>
              </div>
            </div>

            <div id="icon3" className="flex">
              <i class="fa-solid fa-car fa-2x mr-3"></i>
              <div>
                <div className="text-sm font-semibold">Parking</div>
                <div className="text-xs">Bike & Car Parking</div>
              </div>
            </div>
          </div>

          <div id="row2" className="flex p-2 justify-evenly">
            <div id="icon4" className="flex">
              <i class="fa-solid fa-chart-area fa-2x mr-3"></i>
              <div>
                <div className="text-sm font-semibold">Carpet Area</div>
                <div className="text-xs">732 sqft</div>
              </div>
            </div>

            <div id="icon5" className="flex">
              <i class="fa-solid fa-stairs fa-2x mr-3"></i>
              <div>
                <div className="text-sm font-semibold">Floor</div>
                <div className="text-xs">3 out of 7</div>
              </div>
            </div>

            <div id="icon6" className="flex">
              <i class="fa-solid fa-house-chimney fa-2x mr-3"></i>
              <div>
                <div className="text-sm font-semibold">Transaction</div>
                <div className="text-xs">Resale</div>
              </div>
            </div>
          </div>
        </div>

        <div id="details" className="text-start mt-6 text-sm space-y-4">
          <div>Special Price Offered by Owner</div>
          <div>
            Brand New Flat in Brand new Society available for immediate
            possesion.
          </div>
        </div>
      </div>
    </div>
  );
}
